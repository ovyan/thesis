...
def get_bitcoin_data(round=1):
    ts = TimeSeries.from_json('./datasets/bitcoin/bitcoin.json')
    data = ts.df['USD/Bitcoin'].values
    annotations = None
    with open('annotations.json') as f:
        d = json.load(f)
        annotations = d['bitcoin']
