...
if __name__ == "__main__":
    ...
    result = 1 - spatial.distance.cosine(vectors[i - 1], vectors[i])
    cos_sims.append(result)

    plt.figure(figsize=(8, 5))
    plt.plot(cos_sims, label="Moving Cosine Similarity")
    cos_sims = np.array(cos_sims)
    thresh = cos_sims.mean() - 3 * cos_sims.std()
    plt.axhline(y=thresh, color='r', linestyle='--', label="Threshold")
    plt.legend()
    plt.title("Cosine similarity among neighbors")
